...
//使用方法： gcc -o main main.c QR_Encode.c
 ...
 */
int bmp_gen_test(char *fileName, uint32_t width, uint32_t height, uint32_t color, uint8_t *p)
{
    FILE *fp;
    uint32_t i, j;
...
int main(int argc, char *argv[])
{
    ...
    sprintf(bmpName, "%s_%d_%d_%s.bmp", name, width, height,"qrcode_demo");
    printf("bmpName: %s\n", bmpName);

    ret = bmp_gen_test(bmpName, width, height, color, "qrcode_demo");
